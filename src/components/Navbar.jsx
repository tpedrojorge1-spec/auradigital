...
export default function Navbar() {
  ...
  // Detecta se está em subpágina (não na home)
  ...
  // Se estiver em subpágina, links vão para /#secao; na home, vão para #secao
  const navLinks = [
    { label: "Início", href: isSubPage ? "/" : "#hero" },
    { label: "Serviços", href: isSubPage ? "/#servicos" : "#servicos" },
    { label: "Planos", href: isSubPage ? "/#planos" : "#planos" },
    { label: "Avaliações", href: isSubPage ? "/#avaliacoes" : "#avaliacoes" },
    { label: "Contato", href: isSubPage ? "/#contato" : "#contato" },
  ];

  const verPlanosHref = isSubPage ? "/#planos" : "#planos";
  const logoHref = isSubPage ? "/" : "#hero";
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-black/80 backdrop-blur-xl border-b border-purple-900/30 shadow-lg shadow-purple-900/10" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((l) => (
            <a
              key={l.label}
              href={l.href}
              className="font-inter text-xs font-medium tracking-widest text-white/60 hover:text-white transition-all duration-300 uppercase relative group"
            >
              {l.label}
        ...
        <div className="hidden md:flex items-center gap-4">
          {/* Instagram */}
          <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className="group relative">
          ...
          {/* WhatsApp */}
          <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="group relative">
      ...
      {menuOpen && (
        <div className="md:hidden bg-black/95 backdrop-blur-xl border-t border-purple-900/30 px-6 py-4 space-y-3">
          ...
          <a href="/meus-pedidos" onClick={() => setMenuOpen(false)}
